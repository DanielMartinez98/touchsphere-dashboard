    ...
    <div className="flex flex-col gap-1.5">
      <div className="flex items-center gap-2">
        <button type="button" onClick={toggle}
          className={`px-3 py-1.5 rounded-full text-sm font-medium ${active ? 'bg-green-500 text-black' : 'bg-white/[0.06] text-white/60 active:bg-white/10'}`}>
          🙋 Only mine{me && active ? ` · ${me.name}` : ''}
        </button>
        {meId && !picking && (
          <button type="button" onClick={() => setPicking(true)}
            aria-label="Change who 'me' is"
            className="text-sm text-white/30 active:text-white/60 px-1">change</button>
        )}
      </div>
