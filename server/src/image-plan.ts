...
export interface PlanStep {
  ...
  styleLabel?: string
  /**
   * Hold the source's lines with a ControlNet while repainting. Default true
   * for part/whole steps; the planner turns it off only for a change that IS
    ...
    lines.push(
      ...
      ...styles.map(st =>
        `- ${st.id} — "${st.label}": ` + (st.register === 'mixed'
          ? 'booru tags for any named character with its series tag right after it ("1girl, sakura haruno, naruto") then the series’ own creator as "@first last" when certain and "official style", so the character looks exactly as in the series (no other artist, medium or era tags unless asked), the rest as tags or one or two sentences; an illustration model — never ask it for realism'
          : st.register === 'tags'
