...
- **[hooks/useServerEvents.ts](client/src/hooks/useServerEvents.ts)** — one shared `EventSource` on `/api/system/events` with `subscribe(event, cb)`. Both the restart signal and guide progress ride it; a per-hook EventSource would cost the Pi a held-open request and a heartbeat per widget
