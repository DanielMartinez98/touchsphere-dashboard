...
- **[components/widgets/MediaListWidget/GuideView.tsx](client/src/components/widgets/MediaListWidget/GuideView.tsx)** — the **game guide**, full screen: a 100% bar, collapsible sections with their own `x/y` counts, tickable steps, and a YouTube walkthrough per section. Reached from the item sheet's Guide button (games only) and mounted as a layer inside the media widget's overlay, like the cover/rename sheets. Data comes from [hooks/useGuides.ts](client/src/hooks/useGuides.ts): `useGuides()` for the per-game summaries that draw the row/pill bars, `useGuide(itemId)` for the document itself. Both follow the server's `guide` SSE event, which is what makes a generation *fill in while you watch* rather than appear all at once. Videos are handed to the existing [BrowserOverlay](client/src/components/BrowserOverlay.tsx) via `openBrowse`
